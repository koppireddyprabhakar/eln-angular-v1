...
export class AuditlogService {
 ...
 getAuditLogsByUserId(id) {
    const url = `${elnEndpointsConfig.endpoints['getAuditlogsByUserID']}?userId=${id}`;
    return this.http
      .get<any>(url)
      .pipe(catchError((err: HttpErrorResponse) => this.handleError(err)));
  }
  ...
  downloadAuditPdf(request: { logs: any[], fromDate?: string, toDate?: string, userName?: string }): Observable<Blob> {
    const url = elnEndpointsConfig.endpoints['downloadAuditPdf'];
    return this.http.post(url, request, { responseType: 'blob' }).pipe(
      map(res => res as Blob),
